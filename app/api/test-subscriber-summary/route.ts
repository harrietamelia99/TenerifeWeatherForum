...
export async function GET(req: NextRequest) {
  ...
  const recentRows = recent.map((s) => {
    ...
    const subs = [s.daily_digest && "Daily", s.monthly_newsletter && "Monthly"].filter(Boolean).join(" + ");
    return `<tr>
  ...
  const html = `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"/><title>Subscriber Summary</title></head>
<body style="margin:0;padding:0;background:#f0f4f8;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Arial,sans-serif;">
  ...
  <table role="presentation" width="100%" style="max-width:560px;" cellpadding="0" cellspacing="0" border="0">
    <tr><td style="background:linear-gradient(135deg,#053f5c 0%,#429ebd 100%);border-radius:16px 16px 0 0;padding:28px 32px;">
    ...
    <tr><td style="background:#ffffff;padding:28px 32px;">
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin-bottom:28px;"><tr>
        <td width="25%" style="padding-right:8px;"><div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:12px;padding:16px;text-align:center;"><p style="margin:0;font-size:11px;text-transform:uppercase;letter-spacing:1px;color:#94a3b8;">Total</p><p style="margin:6px 0 0;font-size:32px;font-weight:700;color:#053f5c;">${total}</p></div></td>
        <td width="25%" style="padding-right:8px;"><div style="background:#f0f9ff;border:1px solid #bae6fd;border-radius:12px;padding:16px;text-align:center;"><p style="margin:0;font-size:11px;text-transform:uppercase;letter-spacing:1px;color:#0369a1;">Daily</p><p style="margin:6px 0 0;font-size:32px;font-weight:700;color:#0369a1;">${daily}</p></div></td>
        <td width="25%" style="padding-right:8px;"><div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:12px;padding:16px;text-align:center;"><p style="margin:0;font-size:11px;text-transform:uppercase;letter-spacing:1px;color:#15803d;">Monthly</p><p style="margin:6px 0 0;font-size:32px;font-weight:700;color:#15803d;">${monthly}</p></div></td>
        <td width="25%"><div style="background:#faf5ff;border:1px solid #e9d5ff;border-radius:12px;padding:16px;text-align:center;"><p style="margin:0;font-size:11px;text-transform:uppercase;letter-spacing:1px;color:#7e22ce;">Both</p><p style="margin:6px 0 0;font-size:32px;font-weight:700;color:#7e22ce;">${both}</p></div></td>
      </tr></table>
      ${recent.length > 0 ? `
      ...
      </table>` : `<p style="color:#94a3b8;font-size:14px;margin-bottom:24px;">No subscribers yet.</p>`}
    ...
    <tr><td style="background:#f0f4f8;border-radius:0 0 16px 16px;padding:16px 32px;text-align:center;">
      <p style="margin:0;font-size:12px;color:#94a3b8;">Tenerife Weather Forum — subscriber report</p>
    </td></tr>
  </table></td></tr></table>
</body></html>`;
  ...
  await resend.emails.send({
    from: FROM_EMAIL,
    to: "harriet@collectivstudio.uk",
    subject: `[TWF] Subscriber Summary — ${monthYear}`,
    html,
  });

  return NextResponse.json({ ok: true, total, daily, monthly, both });
}
